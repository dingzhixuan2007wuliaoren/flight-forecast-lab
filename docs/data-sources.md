...
| 航线市场历史价格 | SerpApi Google Flights 严格搜索响应可选的 `price_insights.price_history`；复用当次响应，不增加 API 调用 | 只接受与当次起点、终点、计划出发日期和舱位一致的完整美元时间序列，并明确不是具体航班/销售方历史；缺失或格式异常时整段省略，不用模型补造 |
| 景点介绍、照片与评分 | OpenStreetMap 身份与坐标；同一 Wikidata/Wikipedia 实体的介绍；严格验证的 Wikimedia Commons 图片、作者、许可和署名；评分能力目录含 12 个可身份匹配来源 | 没有合格照片时只显示明确标注的精确坐标地图回退；`catalogued` 来源不表示已配置或已查询，页面只显示真实返回且通过同一景点身份匹配的评分 |
| 酒店报价、房型与跨平台评价 | 用户显式操作时按顺序调用 [SerpApi Google Hotels](https://serpapi.com/google-hotels-api) / [Property Details](https://serpapi.com/google-hotels-property-details) 与 [SearchAPI.io Google Hotels](https://www.searchapi.io/docs/google-hotels-api) / [Property](https://www.searchapi.io/docs/google-hotels-property-api)。SearchAPI.io 与其机票来源共用 100 次 lifetime 本地硬上限；SerpApi 继续共用结算周期/小时账本 | 仅在名称、紧邻坐标、provider 房源身份、正价格和安全公开路径均通过时显示；按来源记录 available/no_results/processing/quota/auth/provider/invalid 状态。只有所有已配置来源均完整返回空结果时才报告无结果；模糊、多重匹配、失败来源或字段缺失不用其他酒店或估算数据补位 |
| 公共交通 | 首次加载只用无需密钥的 [Transitous MOTIS API](https://transitous.org/api/)；无完整行程时可显示 OpenStreetMap 返回的两端附近真实线路关系参考。只有用户显式发送 `include_live_transit=true` 时，才允许用现有 `SERPAPI_API_KEY` 回退到 [SerpApi Google Maps Directions](https://serpapi.com/google-maps-directions-api) | SerpApi 与票价/酒店共用配额账本，使用异步 Search ID 轮询并最多受控重提一次。OSM 参考不证明机场与目的地连通，也不生成时刻或时长；完整行程只显示提供商返回值 |
