...
共享这项额度。系统解析四个舱位搜索实际返回的合格候选，并在账户实际剩余的结算周期和小时额度内
为全部候选原子预留二次验证；只有真实额度不足时，未获预留的候选才标为 `partial` / `quota_limited`。
同一航程和舱位有多个已验证销售方时只保留最低价。`deep_search=true` 与
...
For each comparison, SerpApi, SearchAPI, and released Ignav request booking-option verification for
...
完成一次航线比较后，严格航班列表上方会出现“目的地景点 / Attractions”和“目的地酒店 / Hotels”两个入口。景点分为地标、博物馆、自然公园、娱乐和购物，住宿分为酒店、青年旅舍、民宿、汽车旅馆和公寓。景点查询以 Nominatim 确认的机场服务城市中心为圆心，先查询 5 公里种子范围，再将 30 公里范围分成九个可独立重试的 Overpass 区块；酒店先查询 1 公里种子范围，再使用 5 公里四分块及 15/30 公里渐进范围。每个区块会在三个固定公共 Overpass 副本间回退；副本数量不等于独立数据源数量。系统保留成功区块并最多返回 300 个具名、带坐标和永久 OSM 链接的真实地点。完整结果缓存 24 小时，缺失区块、供应商截断或达到安全上限的部分结果只缓存 10 分钟，页面明确显示覆盖缺口。景点介绍优先采用同一 OSM 地点精确绑定的 Wikipedia/Wikidata 内容；没有百科介绍时只依据 OSM 标签生成可追溯的事实摘要。景点照片只接受可验证的 Wikimedia Commons 文件、作者、许可和署名；没有合格照片时详情页显示明确标注“不是照片”的精确坐标 OpenStreetMap 预览。评分能力目录包含 12 个来源，但 `catalogued` 只表示身份匹配适配目录，不代表已配置或已查询；页面只渲染真实返回并通过同一景点身份匹配的评分。30 公里、300 条和公共来源本身仍是明确边界，系统不会把它宣称为商业平台的全城市全量。

After a route comparison, Attractions and Hotels appear above the strict-flight list. Attractions use a 5 km seed plus nine independently retryable cells over the 30 km city-centred area. Hotels use a 1 km seed, four 5 km cells, and progressive 15/30 km expansion. Every part can fail over across three fixed public Overpass replicas; replica count is not presented as independent-source count. Successful parts are retained, and at most 300 real named, geolocated places with permanent OSM evidence links are returned. Complete results cache for 24 hours, while missing parts, provider truncation, or a safety cap produce explicit partial coverage cached for only 10 minutes. Introductions prefer Wikipedia/Wikidata tied to the exact OSM entity; otherwise a clearly sourced OSM-tag factual summary is used. Attraction photos require verified Commons identity, author, licence, and attribution; an exact-coordinate OSM preview is labelled separately when no valid photo exists. The 12-source rating capability catalogue does not imply that catalogue-only adapters are configured or queried. Only source-returned scores that pass exact-place matching are rendered. The 30 km radius, 300-record safety cap, and public-source coverage remain explicit boundaries rather than a claim of complete commercial city inventory.
...
酒店页的基础地点列表不会消耗酒店报价额度。用户填写入住日、退房日和成人数并点击“查询真实资料”后，后端按顺序尝试已配置的 [SerpApi Google Hotels](https://serpapi.com/google-hotels-api) 与 [SearchAPI.io Google Hotels](https://www.searchapi.io/docs/google-hotels-api)；额度耗尽、认证失败、限流、处理中和供应商错误会被分别记录并回退到下一来源。单个来源只有遇到传输异常或 HTTP 408/425/5xx 才允许在再次预留额度后受控重试一次；处理中造成的重提与瞬态重试合计也绝不超过两次业务请求。只有所有已配置来源都成功完成且均为空时，才报告“没有可验证住宿”。SearchAPI.io 酒店调用与其严格机票调用共用 100 次 lifetime 本地硬上限，而 SerpApi 酒店与机票继续共用结算周期/小时账本。Google Hotels 列表中的酒店使用供应商房源标识读取详情；OpenStreetMap 酒店必须先通过精确酒店名称与紧邻坐标重新找到唯一同一房源，任何模糊或多重匹配都拒绝。服务器不会持久化房源令牌；系统不以估算价格、模糊酒店或不安全链接填补失败。

详情页按真实返回值列出不同平台的房型名称、每晚/总价、税费前金额、住客数、床型、早餐、免费取消期限、包含项目和安全预订链接；价格、早餐和取消权益必须来自同一个具体费率行，绝不把房型最低价绑定到另一个缺价套餐的权益。同时按 Google、Tripadvisor、Trip.com 等供应商实际返回的平台显示评分、评价数量、一个来源评价摘录及原评价链接。列表中的部分房型或 Google 聚合评分不会被误当成完整详情；用户显式查询仍会读取严格匹配的 Property Details。只有酒店名称、坐标和供应商房源标识都证明是同一家酒店时才合并这些证据。房源令牌、API key、SerpApi 内部链接和原始错误不会进入缓存、前端或日志；本地脱敏详情缓存一小时。未返回的房型、平台或字段保持“数据源未提供”，税费和最终库存仍以结账页为准。

The base place list consumes no hotel-quote allowance. An explicit Check real details action tries configured [SerpApi Google Hotels](https://serpapi.com/google-hotels-api) and [SearchAPI.io Google Hotels](https://www.searchapi.io/docs/google-hotels-api) in order. Exhausted allowance, authentication, rate limiting, processing, and supplier failures are classified separately before failover. A provider gets one separately reserved controlled retry only for transport errors or HTTP 408/425/5xx; processing resubmission and transient retry together can never exceed two business requests. “No verifiable accommodation” is returned only when every configured source completes with no result. SearchAPI.io hotel and strict-flight calls share its 100-call lifetime local hard wall, while SerpApi hotel and flight calls share the provider billing-window/hourly ledger. A Google Hotels item uses its provider property identity; an OpenStreetMap hotel must resolve to exactly one result with the same normalized name and tightly matching coordinates. Fuzzy or ambiguous matches, estimated prices, and unsafe booking links remain rejected.
...
仓库根目录的 `render.yaml` 会创建一个单实例 Render Docker Web Service。镜像直接打包已验证的 `artifacts/demo`，不会在构建期间重新训练；运行时监听平台提供的 `PORT`。首次创建 Blueprint 时，Render 会要求输入 `SITE_ACCESS_PASSWORD`，并为 SerpApi、SearchAPI 与隔离中的 Ignav 提供可选的托管密钥槽；所有凭据都只能保存在托管环境变量中，绝不能写入 Git。

The root `render.yaml` creates a single-instance Render Docker web service. The image packages the validated `artifacts/demo` instead of retraining during the build, and it listens on the platform-provided `PORT`. During initial Blueprint creation, Render prompts for `SITE_ACCESS_PASSWORD` and exposes optional managed secret slots for SerpApi, SearchAPI, and quarantined Ignav. Every credential belongs only in managed environment variables and must never be committed.

除 `/health` 和 `/ready` 外，部署在设置 `SITE_ACCESS_PASSWORD` 后会使用 HTTP Basic Auth 保护全部页面与 API，默认用户名为 `flight`。Blueprint 使用 `auto` 严格报价模式，按顺序运行已配置且获准参与的来源；Ignav 默认仍处于严格隔离。SearchAPI/Ignav 的终身额度账本位于临时文件系统，服务重建后本地计数不能代替供应商账户计数，因此部署者必须以供应商账户实际余额为准。天气、新闻和无需密钥的机场代理数据仍会自动获取。Render 免费服务会休眠且文件系统是临时的，因此适合个人演示，不应视为高可用生产部署。

With `SITE_ACCESS_PASSWORD` set, every page and API except `/health` and `/ready` is protected by HTTP Basic Auth; the default username is `flight`. The Blueprint uses strict-fare `auto` mode and runs every configured, released source in order; Ignav remains quarantined by default. SearchAPI/Ignav lifetime ledgers live on an ephemeral filesystem, so a service rebuild cannot turn local counters into evidence of provider-account balance; deployment must defer to the actual provider allowance. No-key weather, news, and airport proxy data remain automatic. Render's free service sleeps when idle and has an ephemeral filesystem, so this is a personal demo deployment rather than a highly available production service.
